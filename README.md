...
The smoke phase validates that the frontend can construct the Flask application and serve both `/` and `/.well-known/openid-credential-issuer` through the in-process test client after refreshing the local `.env` configuration.

If the smoke test fails, the script exits non-zero and attempts to stop any process already listening on the expected issuer frontend port (`5003` by default) so a failed health signal does not leave a stale listener behind.
